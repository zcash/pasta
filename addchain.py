...
s10   = sch.sqr(s1)
s11   = sch.mul(s10, s1)
s101  = sch.mul(s11, s10)
s111  = sch.mul(s101, s10)
s1001 = sch.mul(s111, s10)
s1011 = sch.mul(s1001, s10)
sa    = sch.sqrmul(s1, 129, s1)
sb    = sch.sqrmul(sa,  7, s1001)
sc    = sch.sqrmul(sb,  5, s11)
sd    = sch.sqrmul(sc,  2, s1)
se    = sch.sqrmul(sd,  4, s11)
sf    = sch.sqrmul(se,  6, s111)
sg    = sch.sqrmul(sf,  3, s111)
sh    = sch.sqrmul(sg, 10, s1001)
si    = sch.sqrmul(sh,  4, s1001)
sj    = sch.sqrmul(si,  5, s1001)
sk    = sch.sqrmul(sj,  4, s101)
sl    = sch.sqrmul(sk,  4, s1)
sm    = sch.sqrmul(sl,  4, s1011)
sn    = sch.sqrmul(sm,  4, s1011)
so    = sch.sqrmul(sn,  5, s11)
sp    = sch.sqrmul(so,  4, s1)
sq    = sch.sqrmul(sp,  5, s11)
sr    = sch.sqrmul(sq,  4, s111)
ss    = sch.sqrmul(sr,  5, s1011)
st    = sch.sqrmul(ss,  3, s1)
su    = sch.sqr(st, 4)
assert su == s, format(su, 'b')
print(sch.cost())
